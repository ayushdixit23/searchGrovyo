...
const CommunityPost = forwardRef(({ d, i, title, handleLike, data, setShareValue, setShare }, ref) => {
	const [showComments, setShowComments] = useState(false)
	return (
		<>
			{showComments && <div>
				<Comments id={data?.id} dp={data?.dp} fullname={data?.fullname} postId={d?.posts?._id} />
			</div>}
			<div
				ref={ref}
			...
			>
				...
				<div className="bg-white dark:bg-bluedark p-2 max-w-[360px] rounded-xl">
					...
					<div
						className={`h-[200px] mt-2 rounded-2xl bg-slate-200  ${d?.urls.length > 1
										...
										<div className="h-full w-full min-w-[320px] ">
											{
												f?.type.startsWith("image") ? (
													...
													<div className="flex w-full object-cover h-full">
														<VideoPlayer key={i} src={f?.content} width={"100%"} height={"h-full"} />
													</div>
									...
									<div className="h-full w-full min-w-[320px] ">
										{
											d?.urls[0]?.type.startsWith("image") ? (
												...
												<div className="flex w-full object-cover h-full">
													<VideoPlayer key={i} src={d?.urls[0]?.content} width={"100%"} height={"h-full"} />
												</div>
					...
					<div className=" px-2 mt-1  py-1 w-[100%] rounded-lg flex items-center">
						...
						<div className="flex gap-2">
							<div onClick={() => handleLike(d?.posts?._id, d?.liked)} className={`  dark:bg-graydark flex justify-center rounded-xl ${d?.liked ? 
								"bg-[#e0a648] dark:text-black dark:bg-yellow-300 text-white" : "bg-[#f4f4f4]"} items-center gap-1 p-2 `}>
								<PiHandsClapping />
								<div className="text-[12px]">{d?.posts?.likes}</div>
							</div>
							<div onClick={() => 
								{ setShareValue(`http://localhost:3000/main/feed/newForYou/${d?.posts?.community?._id}#${d?.posts?._id}`); setShare(true) }} 
